...
public:
	...
	void ExchangeSort(string StudentID1, size_t Index1, string StudentID2, size_t Index2);
	void BubbleSort(int n, int pos, int n2, int pos2, bool& ordenado);
};
template<class T>
...
template<class T>
void DoubleCircularLinkedList<T>::for_all(std::function<void(T)> f) {
	DoubleNode<T>* Auxiliary = FirstNode;
	for (size_t i = 0; i < Size; ++i) {
		f(Auxiliary->getObject());
		Auxiliary = Auxiliary->getNextNode();
	}
}
template<class T>
void DoubleCircularLinkedList<T>::FisherYatesShuffle() {
	T Auxiliary;
	for (size_t i = Size - 1; i > 0; i--) {
		int j = (rand() % (i + 1));
		Auxiliary = getDoubleNode(i)->getObject();
		getDoubleNode(i)->setObject(getDoubleNode(j)->getObject());
		getDoubleNode(j)->setObject(Auxiliary);
	}
}
template<class T>
void DoubleCircularLinkedList<T>::ExchangeSort(string StudentID1, size_t Index1, string StudentID2, size_t Index2) {
	T aux;
	if (StudentID1 > StudentID2)
	{
		aux = getDoubleNode(Index1)->getObject();
		getDoubleNode(Index1)->setObject(getDoubleNode(Index2)->getObject());
		getDoubleNode(Index2)->setObject(aux);
	}
}
template<class T>
void DoubleCircularLinkedList<T>::BubbleSort(int n, int pos, int n2, int pos2, bool& ordenado)
{
	if (n < n2)
	{
		T aux;
		aux = getDoubleNode(pos)->getElemento();
		getDoubleNode(pos)->setElemento(getDoubleNode(pos2)->getElemento());
		getDoubleNode(pos2)->setElemento(aux);
		ordenado = false;
	}
}
